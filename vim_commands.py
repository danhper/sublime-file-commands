...
class VimOpenFile(sublime_plugin.WindowCommand):
  ...
  def run(self):
    ...
    self.input_panel = self.window.show_input_panel("Open file",
                                                     self.current_path,
                                                     self.on_done,
                                                     self.on_change,
                                                     None)
  ...
  def on_done(self, text):
    try:
      if os.path.isfile(text):
        if text.endswith('.sublime-project') and not self.settings.get('open_project_as_file'):
        ...
        if os.path.isdir(dirname):
          ...
          if not self.settings.get('auto_create_dir', False):
            self.set_error("direcotry {0} does not exist".format(dirname))
          else:
            os.mkdir(dirname)
          self.window.open_file(text)
    except:
  ...
  def on_change(self, text):
    ...
    if text.endswith('\t'):
      dirname = os.path.dirname(text)
      basename = os.path.basename(text).strip()
      self.files = [f + '/' if os.path.isdir(dirname + '/' + f)
                            else f
      ...
      self.window.show_quick_panel(self.files, self.update_path)
  ...
  def update_path(self, index):
    ...
    if os.path.isfile(current_path) and self.settings.get('open_file_on_complete', False):
      self.on_done(current_path)
      self.window.run_command("hide_panel")
    else:
      edit = self.input_panel.begin_edit()
      self.input_panel.replace(edit, input_region, current_path)
